...
class ChannelTrace(ParameterWithSetpoints):
    ...
    def __init__(self, name: str, channel: int, **kwargs: Any) -> None:
        ...
        if channel not in self._valid_channels:
            raise ValueError('Invalid channel specifier. SR810 only has '
                             'channels 1 and 2.')

        if not isinstance(self.root_instrument, SR810):
            raise ValueError('Invalid parent instrument. ChannelBuffer '
                             'can only live on an SR810.')

        self.channel = channel
        self.update_unit()

    def update_unit(self) -> None:
        assert isinstance(self.root_instrument, SR810)
    ...
    def get_raw(self) -> ParamRawDataType:
        ...
        if N == 0:
            raise ValueError('No points stored in SR810 data buffer.'
...
class ChannelBuffer(ArrayParameter):
    ...
    def __init__(self, name: str, instrument: SR810, channel: int) -> None:
        """
        Args:
            name: The name of the parameter
        ...
        if not isinstance(instrument, SR810):
            raise ValueError('Invalid parent instrument. ChannelBuffer '
                             'can only live on an SR810.')
    ...
    def prepare_buffer_readout(self) -> None:
        ...
        get the right units
        """
        assert isinstance(self.instrument, SR810)
    ...
    def get_raw(self) -> ParamRawDataType:
        ...
        if self.shape[0] != N:
            raise RuntimeError(
                f"SR810 got {N} points in buffer expected {self.shape[0]}"
            )
        return numbers
...
class SR810(VisaInstrument):
    """
    QCoDeS driver for the Stanford Research Systems SR810 Lock-in Amplifier.
    """
    ...
    def _get_ch_ratio(self, channel: int) -> str:
        val_mapping = {1: {0: 'none',
                       ...
                       2: {0: 'none',
                           1: 'Aux In 3',
                           2: 'Aux In 4'}}
        resp = int(self.ask(f'DDEF ?').split(',')[1])
    ...
    def _set_ch_ratio(self, channel: int, ratio: str) -> None:
        ...
        disp_val = int(self.ask(f'DDEF ?').split(',')[0])
    ...
    def _get_ch_display(self, channel: int) -> str:
        val_mapping = {1: {0: 'X',
                       ...
                       2: {0: 'Y',
                           ...
                           3: 'Aux In 3',
                           4: 'Aux In 4'}}
        resp = int(self.ask(f'DDEF ?').split(',')[0])
    ...
    def _set_ch_display(self, channel: int, disp: str) -> None:
        ...
        ratio_val = int(self.ask(f'DDEF ?').split(',')[1])
